...
1 & Convertidor Step-down LM2596 & Regulador de potencia ajustable regulado a 5.0V para alimentar la ESP32, sensores y servomotores en paralelo \\
1 & Porta Pilas Dual 2x18650 (2 celdas Li-ion) & Un único pack de 2x18650 proveyendo 7.4V de entrada al regulador \\
...
GPIO 23, 17, 15, 13 & Servos Coxa & Salida PWM & Control de cadera (FR, FL, RL, RR) \\
Vin (ESP32) & LM2596 Out & Alimentación & Tensión de entrada regulada (5.0V) \\
...
Los picos de corriente transitorios que ocurren al iniciar la marcha del cuadrúpedo pueden generar caídas bruscas en la tensión lógica de alimentación (\textit{brownouts}), reiniciando la ESP32. Al reducir los servos a 4 unidades, el consumo de corriente total disminuye considerablemente, permitiendo simplificar la electrónica a un único convertidor reductor \textbf{LM2596} y un solo pack de baterías 18650. 

El LM2596 se regula a exactamente \textbf{5.0V} y alimenta tanto la ESP32 (pin Vin) como los sensores (HC-SR04, MPU6050, Buzzer) y los 4 servomotores de cadera en paralelo. Es fundamental unificar todas las referencias de tierra (\textbf{GND}) en un nodo común para evitar derivas de voltaje y lecturas corruptas en los sensores.
...
\begin{enumerate}
    ...
    \item Conectar el pack 2x18650 de 7.4V a la bornera de entrada del LM2596. Antes de conectar la ESP32 y los servos, verificar con un multímetro que la salida del LM2596 esté regulada a exactamente 5.0V. Al encender el interruptor principal, verificar que los leds de la ESP32 y de la IMU MPU6050 se encienden, y que los servos ejecutan el comando inicial de pose de reposo segura.

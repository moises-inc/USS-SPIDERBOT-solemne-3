...
1 & UBEC Regulador Step-down 5V/3A & Regulador de potencia para alimentar los servos y la ESP32 \\
1 & Pack Baterías 2S 14500 (7.4V, 2200mAh) & 2 celdas de litio recargables de tamaño AA conectadas en serie \\
...
GPIO 13, 15, 4, 23 & Servos Coxa (FR, FL, RL, RR) & Salida PWM & Control directo de posición para articulación de cadera \\
GPIO 12, 2, 5, 25 & Servos Fémur (FR, FL, RL, RR) & Salida PWM & Control directo de posición para articulación de rodilla \\
Vin & UBEC Out (5V) & Alimentación & Tensión de entrada regulada para ESP32 \\
3.3 V (ESP32) & MPU6050 (IMU) & Alimentación VCC & Tensión lógica para sensor inercial \\
...
Los picos de corriente transitorios que ocurren al iniciar la marcha del cuadrúpedo pueden generar caídas bruscas en la tensión lógica de alimentación (\textit{brownouts}), reiniciando la ESP32. Para evitar esto, se utiliza un regulador de voltaje reductor \textbf{UBEC (5V/3A)} externo alimentado por el pack de 2 baterías de litio 14500 en serie (7.4V nominales). Su salida de 5V se conecta al riel de potencia de los servos y al pin Vin de la ESP32 de forma aislada. Es fundamental unificar todas las referencias \textbf{GND} en un nodo común para evitar deriva de voltaje y lectura corrupta en la IMU y los servos.
...
\codigo{establecer\_angulo\_pata()} & \codigo{idx, c\_ang, f\_ang} / Sin retorno & Aplica límites físicos de seguridad, calcula compensaciones y escribe directamente en los pines PWM de la ESP32. \\
...
\midrule
1. INICIO & Carga de configuración de I2C. Inicializa la IMU MPU6050 y la modulación PWM directa para los 8 servos. Invoca \codigo{pos\_reposo()}. Lanza las corrutinas asíncronas concurrentes. \\
...
\begin{enumerate}
    ...
    \item Conectar la batería Li-ion 2S 14500 de 7.4V a la bornera de entrada de potencia del UBEC. Al encender el interruptor principal, verificar que los leds de la ESP32 y de la IMU MPU6050 se encienden, y que los servos ejecutan el comando inicial de pose de reposo segura.

...
\begin{itemize}
    ...
    \item \textbf{Estabilización Activa Proporcional:} Corrección de postura inyectando compensaciones angulares dinámicas a los servos de rodilla (fémures) de las patas en contacto.
...
1 & Módulo Expansor PCA9685 I2C & Controlador PWM de 16 canales de 12 bits para los servos \\
...
1 & UBEC Regulador Step-down 5V/3A & Regulador aislado de potencia para alimentar la fuerza de servos \\
1 & Batería Li-ion 2S 7.4V (2200mAh) & Fuente de potencia primaria del sistema \\
...
El bus físico I2C de la ESP32 se comparte en paralelo entre la IMU MPU6050 y el expansor de servomotores PCA9685. El sonar HC-SR04 y el buzzer activo se rutean a GPIOs digitales específicos:
...
\midrule
GPIO 21 & PCA9685 / MPU6050 & SDA (I2C) & Bus de datos serie compartido \\
GPIO 22 & PCA9685 / MPU6050 & SCL (I2C) & Bus de reloj serie compartido \\
...
PCA Canales 0-3 & Servos Coxa (0-3) & Salida PWM & Modulación de posición para caderas \\
PCA Canales 4-7 & Servos Fémur (0-3) & Salida PWM & Modulación de posición para muslos \\
Vin & UBEC Out (5V) & Alimentación & Tensión de entrada de control para ESP32 \\
3.3 V (ESP32) & MPU6050 / PCA9685 & Alimentación VCC & Tensión lógica para sensores y bus de señales \\
GND (Unificado) & Todos los componentes & Tierra común & Referencia negativa unificada del sistema \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Nota sobre el Aislamiento de Potencia y GND Común}
Los picos de corriente transitorios que ocurren al iniciar la marcha del cuadrúpedo pueden generar caídas bruscas en la tensión lógicas de alimentación (*brownouts*), reiniciando la ESP32. Para evitar esto, se utiliza un regulador **UBEC (5V/3A)** externo. Su salida se conecta directamente a la bornera de alimentación del PCA9685, aislando la corriente de los servos. Es fundamental unificar todas las referencias **GND** en un nodo común para evitar deriva de voltaje y lectura corrupta en la IMU y los servos.
...
\midrule
\codigo{pos\_reposo()} & Sin parámetros / Sin retorno & Envía todos los servomotores a sus ángulos estáticos de seguridad. \\
\codigo{establecer\_angulo\_pata()} & \codigo{idx, c\_ang, f\_ang} / Sin retorno & Aplica el factor de conversión a pasos de 12 bits y escribe en el canal correspondiente del PCA9685. \\
\codigo{mover\_suave\_ciclo()} & \codigo{c\_t, f\_t, pasos, delay} / async & Realiza interpolación lineal entre la posición actual de los servos y sus objetivos para evitar tirones. \\
\codigo{caminar\_adelante()} & async / Sin retorno & Ejecuta la secuencia Crawl Gait en 5 pasos llamando a \codigo{mover\_suave\_ciclo()}. \\
...
\codigo{locomotion\_loop()} & async / Corrutina cíclica & Evalúa los comandos de marcha e inyecta dinámicamente la compensación inercial proporcional si $|\theta| > 3^\circ$. \\
...
\midrule
1. INICIO & Carga de configuración de I2C. Inicializa PCA9685 e IMU MPU6050. Invoca \codigo{pos\_reposo()}. Lanza las corrutinas asíncronas concurrentes. \\
...
El circuito y el firmware completo del cuadrúpedo USS SpiderBot fueron modelados y validados digitalmente en Wokwi. Dado que las cunas reales del hardware están compuestas de múltiples esclavos I2C, la simulación implementa una inicialización de autodetención o autodesvío (fallback) para verificar la lógica en el simulador en ausencia del controlador PCA9685 real.
...
\begin{enumerate}
    ...
    \item Crear un nuevo proyecto basado en la tarjeta de desarrollo **ESP32 DevKit V1**.
    ...
    \item Presionar el botón de inicio de simulación (**▶ Start Simulation**).
    \item Interactuar con el panel interactivo modificando las distancias de proximidad del HC-SR04 y observando el comportamiento del buzzer y la oscilación de los 8 servomotores simulados.
\end{enumerate}
...
\url{https://wokwi.com/projects/uss-spiderbot-simulation}
...
\begin{enumerate}
    ...
    \item Abrir el entorno integrado Thonny IDE y seleccionar en la esquina inferior derecha el intérprete de ejecución **MicroPython (ESP32)**.
    ...
    \item Conectar la batería Li-ion de 7.4V a la bornera de entrada de potencia del UBEC. Al encender el interruptor principal, verificar que los leds del driver PCA9685 y de la IMU MPU6050 se encienden, y que los servos ejecutan el comando inicial de pose de reposo segura.
...
\begin{enumerate}
    ...
    \item Utilice los mandos direccionales interactivos en pantalla o presione las teclas del computador (**W** para avanzar, **S** para retroceder, **A** para girar a la izquierda, **D** para girar a la derecha y **Espacio** para frenar de inmediato).
...
\begin{itemize}
    ...
    \item \textbf{Infill (Relleno):} $30\%$ en patrón **Gyroid** por su rigidez tridimensional isotrópica.
\end{itemize}
...
% ======================================================
\section{Evidencia Fotográfica del Prototipo}
Este apartado recopila el registro fotográfico de la puesta en marcha física y la verificación empírica del hardware:

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{spiderbot_render.png}
    \caption{Renderizado y maqueta digital de ensamble del chasis doble deck y la protoboard de 400 puntos sobre la placa superior.}
    \label{fig:montaje_general}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{stabilization_response.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_simulation.png}
...
\begin{enumerate}
    ...
    \item El modelado CAD paramétrico en OpenSCAD permitió implementar tolerancias de diseño de precisión para FDM ($0.3\text{ mm}$ snug fit), bolsillos dual-depth que evitan la torsión del cableado, y cerraduras embutidas (*horn locks*) para optimizar la transmisión de torque de los servomotores.
    \item El lazo cerrado proporcional inercial con constante calibrada $K_p = 1.2$ demostró estabilidad estática eficaz sobre inclinaciones dinámicas de hasta $\pm 10^\circ$, respondiendo en menos de $25\text{ ms}$.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Optimizaciones de Red Offline:} Ampliar el rango dinámico del Dashboard web offline habilitando almacenamiento local (*LocalStorage*) para mapeo topológico relativo.
\end{itemize}
